...
class seqRun():
	...
	def getSeqNameByID(self,exptID):
		...
		query='select flowcellID, startDate, completionDate, genomicsLead, dataLocation, indexTagCycles, readCycles,seqRunName from seqRun where seqRunID='+str(exptID)
	...
	#insertSeqRun method
		...
		insQuery="INSERT INTO seqRun (seqRunName,flowcellID,startDate,completionDate,genomicsLead,dataLocation,indexTagCycles,readCycles) "
                vals=" VALUES ('"+self.seqRunName+"','"+str(self.flowcellID)+"','"+str(self.startDate)+"','"+str(self.completionDate)+"','"+self.genomicsLead+"','"+self.dataLocation+"',"+str(self.indexTagCycles)+","+str(self.readCycles)+")"
	...
	def updateDB(self):
                ...
                updateQuery="UPDATE seqRun SET seqRunName='"+self.seqRunName+"', flowcellID='"+str(self.flowcellID)+"', startDate='"+str(self.startDate)+"', completionDate='"+str(self.completionDate)+"', genomicsLead='"+str(self.genomicsLead)+"', dataLocation='"+str(self.dataLocation)+"', indexTagCycles="+str(self.indexTagCycles)+", readCycles="+str(self.readCycles)+" where seqRunID="+str(self.seqRunID)
