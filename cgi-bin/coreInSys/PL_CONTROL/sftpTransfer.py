...
class sftpTransfer:
	...
	def setTransfer(self):
		...
		DBquery="select transferID,transLocation,sftpAccountID,dataStatus from transfer where seqProjectID="+str(self.seqProjectID)
	...
	def setTransLocation(self):
		...
		try:
                        ...
                        print "Unable to load seq project with ID "+str(self.seqProjectID)
                        sys.exit()

		try:
			j=getRunByID(int(i.seqRunID))
		except Exception:
                        print "Unable to get sequencing run"+str(i.seqRunID)
                        sys.exit()
	...
	def restore2demuxDir(self):
		...
		try:
			...
			print "<p>Unable to find transfer details in database.</p>"
			sys.exit()
		...
		if os.path.islink(dataDestPATH):	
			...
			print "<p>Data folder in demux is not a symbolic link. Aborting.</p>"
		...
		if os.path.islink(qcDestPATH):        
			...
			command=os.path.join(self.exec_root,"moveData")+" "+qcSourcePATH+" "+qcDestPATH
                        ...
                        print "<p>QC folder in demux is not a symbolic link. Aborting.</p>"
	...
	def tranfer2sftp(self):
		...
		if len(res)>0:
			print "<p>res=",res[0][0],"</p>"
			if res[0][0]=='sftp':
				print "<p>Data has already been moved to sftp area. Aborting transfer</p>"
				return
			else:
			...
			DBquery="insert into transfer(seqProjectID, transLocation, sftpAccountID,dataStatus) values("+str(self.seqProjectID)+",'"+self.transLocation+"',"+str(self.sftpAccountID)+",'sftp')"
                	res=runQuery(DBquery)
		...
		if os.path.islink(dataSourcePATH):
        		print "<p>Data source directory is a symbolic link. Aborting!<br>"
        		print "(This probably means the data have already been moved to the sftp area)</p>"
		else:
			...
			command=os.path.join(self.exec_root,"makeLink")+" "+self.transLocation+"/processed "+dataSourcePATH
                        p = sub.Popen(command, shell=True, stdout=sub.PIPE,stdin=sub.PIPE).stdout
                        linkRemoved=p.readlines()
                        print "<p>",command,"</p>"
		...
		if os.path.islink(qcSourcePATH):
        		print "<p>QC source directory is a symbolic link. Aborting!<br>"
        		print "(This probably means the QC stats have already been moved to the sftp area)</p>"
		else:
		#if 1:
	...
	def getTransStat(self):
		import runQuery
		DBquery="select transferID, transLocation, sftpAccountID from transfer where seqProjectID="+str(self.seqProjectID)
		...
		if len(res)>0:
			...
			DBquery="select accountLocation, username, userContact, creationDate, accountStatus from sftpAccount where sftpAccountID="+str(self.sftpAccountID)
			res=runQuery.runQuery(DBquery)
			if len(res)>0:
				self.accountLocation=res[0][0]
	...
	def addSftpAccount(self):
		...
		if self.username=="NULL":
			print "<p>Need to generate a username before adding an account</p>"
			return 0
		...
		#print command
		command=os.path.join(self.exec_root,"runSftpSetup")+" " +self.username+" "+self.password+" "+self.accountLocation
		#print "<p>",command,"</p>"
		p = sub.Popen(command, shell=True, stdout=sub.PIPE,stdin=sub.PIPE).stdout
		...
		print "<p>Account created, password="+self.password+"</p>"
