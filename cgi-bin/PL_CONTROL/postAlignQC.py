...
class postAlignQC:
	def __init__(self,i,j):
        ...
        def establishPaths(self):
                ...
                for la in self.i.lanes:
                        ...
                        for sa in la.samples:
                                ...
                                self.currentOutDirName=os.path.join(self.postAlignQCdir,self.currentSample.sampleName)+"_L"+laneNumber
        ...
        def setupAllSamples(self): #This can be altered to be goAllSamples like in the fastQC things once we're automating things.
                for la in self.i.lanes:
                        self.currentLane=la
                        for sa in la.samples:
		...
		try:
			if not os.path.exists(dirToMake):
				...
				os.chmod(dirToMake,0777)
                        ...
                        print "<p>Failed to make directory ",dirToMake,". </p>"
	...
	def createDBentry(self):
		...
		DBinsert="insert into postAlignQC (seqProjectID,sampleID,location,sourceLocation,JID,status)"
                ...
                DBvals=" values("+str(self.i.seqProjectID)+","+str(self.currentSample.sampleID)+",'"+self.currentOutDirName+"','"+self.currentSamplePath+"',"+self.currentJID+",'running')"
	...
	def makeLinksToFiles(self):
		...
		print '<div id="PAQCdetailsHidden" style="display:none">'
        	print """<a href="javascript:swapDisplayed('PAQCdetailsShown','PAQCdetailsHidden')">hide post alignment QC files</a> """
		...
		for t in range(0,len(self.PAQfiles)):			
			...
			if self.PAQfiles[t][-3:]=="bam":
				print '<p><a href="sftp://biocis@sheba.genetics.kcl.ac.uk'+self.postAlignQCdir+'/'+self.PAQfiles[t]+'">'+self.PAQfiles[t]+'</a></p>'
			else:
				print "<p><a href='"+loca+"'>"+self.PAQfiles[t]+"</a></p>"
		print '</div>'	

		print '<div id="PAQCdetailsShown" style="display:inline">'
        	print """<a href="javascript:swapDisplayed('PAQCdetailsHidden','PAQCdetailsShown')">show post alignment QC files</a> """
		print '</div>'
