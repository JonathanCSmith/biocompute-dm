...
class masterProject:
	...
	def makeProjectDir(self):
                import os
		print "<p>Making project dir",self.projectDir,"</p>"
                try:
                        if not os.path.exists(self.projectDir):
                                os.mkdir(self.projectDir)
                                os.chmod(self.projectDir,0777)
                except OSError:
                        print "<p>Failed to make directory ",self.projectDir,". </p>"
		...
		try:
                        ...
                        print "<p>Failed to make directory ",docsDir,". </p>"
	...
	def addDocToDB(self,docDescription,docLocation):
		...
		DBquery="insert into masterProjectDocuments (docDescription, docLocation, masterProjectID) values ('"+docDescription+"','"+docLocation+"','"+str(self.masterProjectID)+"')"
		try:
			res=runQuery.runQuery(DBquery)
		except Exception:
			print "<p>Failed to add document to the database.</p>"
	...
	def checkDocuments(self):
		...
		self.documents=[]
		DBquery="select documentID, docDescription, docLocation from masterProjectDocuments where masterProjectID="+str(self.masterProjectID)+" order by documentID"
	...
	def removeDocumentByID(self,documentID):
		...
		print "<p>",docLocation,"</p>"
		try:
    			os.remove(docLocation)
		except Exception:
			print "<p>Unable to remove file "+docLocation+"</p>"
    			#pass	
