...
CairoPNG(file=paste(outputOfPlots, "coverage.png", sep=""), res=111, width=860*2.02, height=860)
qcdf_melt=melt(qcdf[,c("sampleNames", "cov_1x", "cov_5x", "cov_10x", "cov_20x")])
g = ggplot(data=qcdf_melt)
g = g + geom_bar(mapping=aes(x=sampleNames, y=value, fill=variable),
                 stat="identity",
                 position="identity",
...
g = g + theme(axis.text.x = element_text(size  = 10,
                                         ...
                                         hjust = 1,
                                         vjust = 1))
g = g + ggtitle("Coverage Metrics") + ylab("% of Genome covered")
g
dev.off()
...
CairoPNG(file=paste(outputOfPlots, "capture.png", sep=""), res=111, width=860*2.02, height=860)
qcdf_melt=melt(qcdf[,c("sampleNames", "capture", "capture150")])
g = ggplot(data=qcdf_melt)
g = g + geom_bar(mapping=aes(x=sampleNames, y=value, fill=variable),
                 stat="identity",
                 position="identity",
...
g = g + theme(axis.text.x = element_text(size  = 10,
                                         ...
                                         hjust = 1,
                                         vjust = 1))
g = g + ggtitle("Capture Metrics") + ylab("% of baits covered")
g
dev.off()
