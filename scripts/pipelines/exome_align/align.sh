...
JOBID=$(ssh ${USERNAME}@${HPC_IP} << END
    source /etc/profile;
    JOBID=\$(qsub -t 1-${FILE_COUNT} -v "DATA_FILE="${SAMPLE_CSV}",ref="${ref} -o "${MODULE_OUTPUT_DIRECTORY}" -e "${MODULE_OUTPUT_DIRECTORY}" "${PIPELINE_SOURCE}//align_worker.sh" | cut -d ' ' -f 3);
    echo \$JOBID
END
)
