...
ssh ${USERNAME}@${HPC_IP} << EOF
    ...
    JOBID=\$(qsub -V -N job-${TICKET} -o ${OUTPUT_DIR}/${MODULE}_q_submission_output.log -e ${OUTPUT_DIR}/${MODULE}_q_submission_error.log -wd ${WORKING_DIRECTORY} -v ${VARS} ${SCRIPT_STRING} | cut -d ' ' -f 3);
    echo Job Id: \$JOBID
